...
export default function Home() {
  ...
  const [status, setStatus] = useState("Add evidence to begin. Matching runs locally in your browser.");
  ...
  useEffect(() => { if (!detail && !settingsOpen && !consent && !aiReviewOpen && !profileProposal && !cvDraft) return; const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") { setDetail(null); setSettingsOpen(false); setConsent(false); setAiReviewOpen(false); setProfileProposal(null); setCvDraft(null); setCvAccepted([]); setCvRejected([]); } }; document.addEventListener("keydown", closeOnEscape); return () => document.removeEventListener("keydown", closeOnEscape); }, [detail, settingsOpen, consent, aiReviewOpen, profileProposal, cvDraft]);
  ...
  async function runMatch(event?: FormEvent) { event?.preventDefault(); if (!profile && !combinedText) { setStatus("Build a profile or add evidence first."); return; } setBusy(true); setTab("results"); setStatus("Screening the complete Mitacs corpus..."); try { const evidenceText = profile ? [...profile.skills, ...profile.tools, ...profile.domains, ...profile.methods, ...profile.evidence].join(" ") : combinedText; const rankFilters = { province: filters.province, university: filters.university, research_area: filters.research }; const ranked = rankProjects(publicCorpus.projects, evidenceText, rankFilters); setProjects(ranked.slice(0, 50)); setResultsVersion(profile ? hashText(profileToText(profile)) : hashText(combinedText)); setResultsStale(false); setStatus(`Full-corpus retrieval complete. ${publicCorpus.projects.length} projects were screened.`); } catch (error) { setStatus(error instanceof Error ? error.message : "Matching failed. Your local evidence is preserved."); } finally { setBusy(false); } }
  ...
  function removeShortlist(projectId: string) { const next = shortlist.filter((x) => x.project_id !== projectId).map((item, index) => ({ ...item, rank: index + 1 })); setShortlist(next); saveLocalShortlist(next); setStatus("Removed from the local shortlist."); }
  ...
  return <main className="shell">
    ...
    <div className="workspace-shell">
      ...
      <section className="content">
        ...
        {tab === "profile" && <ProfileReview profile={profile} onChange={updateProfile} onContinue={() => runMatch()} onBack={() => setTab("knowledge")} onImprove={() => openAIReview("profile")} canImprove={Boolean(keyReady && profile && candidateDocuments.some((document) => document.status === "ready"))} hasKey={keyReady} onOpenSettings={() => setSettingsOpen(true)} />}
        ...
        {tab === "shortlist" && <ShortlistWorkspace shortlist={shortlist} projects={projects} selected={selected} onRemoveCompare={toggleCompare} onRemoveShortlist={removeShortlist} onExport={exportMarkdown} onBack={() => setTab("results")} />}
      </section>
    </div>
