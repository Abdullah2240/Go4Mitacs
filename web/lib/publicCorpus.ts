...
export function rankProjects(projects: PublicProject[], evidenceText: string, filters: { province?: string; university?: string; research_area?: string }) {
  ...
  return projects.filter((project) => (!filters.province || (project.province || "").toLowerCase() === filters.province.toLowerCase()) && (!filters.university || (project.university || "").toLowerCase() === filters.university.toLowerCase()) && (!filters.research_area || searchable(project).toLowerCase().includes(filters.research_area.toLowerCase()))).map((project) => {
    const indexed = indexProject(project); const matched = query.filter((term) => indexed.all.has(term)); const matchedWeight = matched.reduce((sum, term) => sum + idf(term), 0);
    ...
    // could only ever lower this signal, since no single project mentions most of a real CV.
    const projectWeight = Array.from(indexed.all).reduce((sum, term) => sum + idf(term), 0) || 1;
    const evidenceCoverage = Math.min(1, matchedWeight / Math.min(queryWeight, projectWeight));
    const titleMatched = query.filter((term) => indexed.fields.title.includes(term));
    const titleWeight = Array.from(new Set(indexed.fields.title)).reduce((sum, term) => sum + idf(term), 0) || 1;
    const titleCoverage = Math.min(1, titleMatched.reduce((sum, term) => sum + idf(term), 0) / Math.min(queryWeight, titleWeight));
    let bm25 = 0;
