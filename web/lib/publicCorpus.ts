...
type IndexedProject = { project: PublicProject; fields: Record<FieldName, string[]>; all: Set<string> };
...
function indexProject(project: PublicProject): IndexedProject { const fields = Object.fromEntries((Object.entries(fieldText(project)) as Array<[FieldName, string]>).map(([name, value]) => [name, terms(value)])) as Record<FieldName, string[]>; return { project, fields, all: new Set(Object.values(fields).flat()) }; }
const corpusIndex = publicCorpus.projects.map(indexProject);
const documentFrequency = new Map<string, number>();
for (const indexed of corpusIndex) for (const term of Array.from(indexed.all)) documentFrequency.set(term, (documentFrequency.get(term) || 0) + 1);
const averageLength: Record<FieldName, number> = { title: 1, research: 1, skills: 1, description: 1 };
for (const field of Object.keys(averageLength) as FieldName[]) averageLength[field] = corpusIndex.reduce((sum, item) => sum + item.fields[field].length, 0) / Math.max(1, corpusIndex.length);
function idf(term: string) { const df = documentFrequency.get(term) || 0; return Math.log(1 + (corpusIndex.length - df + 0.5) / (df + 0.5)); }
...
export function rankProjects(projects: PublicProject[], evidenceText: string, filters: { province?: string; university?: string; research_area?: string }) {
  ...
  return projects.filter((project) => (!filters.province || (project.province || "").toLowerCase() === filters.province.toLowerCase()) && (!filters.university || (project.university || "").toLowerCase() === filters.university.toLowerCase()) && (!filters.research_area || searchable(project).toLowerCase().includes(filters.research_area.toLowerCase()))).map((project) => {
    const indexed = indexProject(project); const matched = query.filter((term) => indexed.all.has(term)); const matchedWeight = matched.reduce((sum, term) => sum + idf(term), 0); const evidenceCoverage = matchedWeight / queryWeight; const titleMatched = query.filter((term) => indexed.fields.title.includes(term)); const titleCoverage = titleMatched.reduce((sum, term) => sum + idf(term), 0) / queryWeight; let bm25 = 0;
    ...
    const bm25Signal = bm25 / (bm25 + 12); const phraseSignal = queryPhrases.length > 1 ? Math.min(1, (Object.keys(fieldWeights) as FieldName[]).reduce((sum, field) => sum + phraseHits(queryPhrases, indexed.fields[field]), 0) / Math.max(1, queryPhrases.length - 1)) : 0; const score = (bm25Signal * 0.55 + evidenceCoverage * 0.25 + phraseSignal * 0.1 + Math.min(1, titleCoverage) * 0.1) * 100; const missingEvidence = query.filter((term) => !indexed.all.has(term)).slice(0, 8);
    return { project_id: project.id, title: project.title, score: Number(score.toFixed(4)), score_breakdown: { bm25_relevance: Number((bm25Signal * 55).toFixed(2)), evidence_coverage: Number((evidenceCoverage * 25).toFixed(2)), phrase_match: Number((phraseSignal * 10).toFixed(2)), title_relevance: Number((Math.min(1, titleCoverage) * 10).toFixed(2)) }, group: score >= 70 ? "ambitious" : score >= 35 ? "strong-fit" : "reliable", matched_evidence: matched.slice(0, 20), missing_evidence: missingEvidence.length ? missingEvidence : ["No major vocabulary gap detected; verify the full project requirements manually"], risks: ["Lexical BM25 evidence signal only; review requirements and eligibility manually"], metadata: { ProjectID: project.id, ProjectTitle: project.title, LanguageUsed: project.language, StartDate: project.start_date, isStartDateFlexible: project.flexible_start, NotesOnStartDate: project.start_notes, Province: project.province, PreferredBackgroundCollection: project.skills_background, "Professor.FirstName": project.supervisor.first_name, "Professor.LastName": project.supervisor.last_name, "Professor.UniversityName": project.university, "Professor.CampusName": project.campus }, source_url: GLOBALINK_PORTAL_URL, text_preview: project.description_preview };
