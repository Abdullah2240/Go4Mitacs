...
Private historical professor references are excluded by default and require a separate checkbox. Keys are saved only in `sessionStorage`, never placed in URLs or localStorage, and removed by Forget Key. Saving a key makes no provider call.
